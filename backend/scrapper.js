  ...
  try {
    ...
    const countriesMap = cheerio(
      ...
      if (i <= 7) {
        ...
        const country_name =
          cheerio(e).find("a").first().text() ||
          cheerio(e).find("td:nth-child(2)").text();
        const total_cases = cheerio(e).find("td:nth-child(3)").text();
    ...
    const countriesTableSorted = countriesTable.sort(compare);
    const countriesTablesSortedFiltered = countriesTableSorted.filter((el) => {
      return el.country_name !== "Total:";
    });
    return countriesTablesSortedFiltered;
